  ...
  return (
    ...
    >
      {items.map((item, idx) => (
        <Link
          onClick={onItemClick}
          className="relative px-4 py-2 text-orange hover:text-white dark:text-neutral-300 transition-colors delay-100"
          key={`link-${idx}`}
          href={item.link}
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 bg-orange dark:bg-neutral-800 rounded-full w-full h-full"
            />
          )}
